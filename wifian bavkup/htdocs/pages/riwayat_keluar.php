...
if ($search !== '') {
    $stmt = $pdo->prepare("SELECT * FROM stok_rusak_keluar WHERE tanggal LIKE ? ORDER BY id DESC");
    $stmt->execute(["%$search%"]);
} else {
    $stmt = $pdo->query("SELECT * FROM stok_rusak_keluar ORDER BY id DESC");
}
...
<div class="p-6 md:p-8 space-y-6 bg-slate-50 min-h-screen">
    ...
    <!-- Header Section -->
    <div>
        <h1 class="text-xl font-bold text-slate-800 tracking-tight">Riwayat Stok Rusak Keluar</h1>
        <p class="text-sm font-normal text-slate-500 mt-0.5">Daftar transaksi barang rusak yang dikeluarkan atau dibuang.</p>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[11px] font-bold text-slate-500 uppercase tracking-wider bg-slate-50/70 border-b border-slate-200/80">
                        ...
                        <th class="py-3.5 px-5">KETERANGAN / TINDAKAN</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-600">
                    <?php if (empty($riwayatKeluar)): ?>
                        <tr>
                            <td colspan="5" class="py-8 text-center text-slate-400 italic">Belum ada riwayat keluar.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($riwayatKeluar as $row): ?>
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-4 px-5">
                                    <span class="bg-rose-50 text-rose-600 px-2.5 py-1 rounded-full text-[10px] font-bold border border-rose-100 inline-flex items-center gap-1">
                                        ↑ <?= htmlspecialchars($row['info'] ?? 'Keluar') ?>
                                    </span>
                                </td>
                                ...
                                <td class="py-4 px-5 text-right font-bold text-rose-600">
